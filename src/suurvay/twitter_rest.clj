...
(defn get-friends
  ...
  [creds identifier]
  (let [user (identifier->map identifier)]
    ;; BREAK I have no idea why this isn't working!!!
    (-> (try-with-creds creds t/friends-ids :params user) ;; BREAK arg
      :body
      :ids)))
...
(sc/defn get-blocks :- #{Identifier}
  ([creds identifier :- (sc/maybe Identifier)]
   (let [base-params {}
         params (if identifier
                  (merge base-params (identifier->map identifier))
                  base-params)]
     (-> (try-with-single-creds creds :params params)
       :body
       :ids
       set))))
